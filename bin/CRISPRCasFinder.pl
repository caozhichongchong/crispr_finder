...
my $vmatchProg = '/scratch/users/anniz44/bin/pro/vmatch-2.3.1-Linux_x86_64-64bit/vmatch2';
if($vmatchProg){
  print "vmatch2 is...............OK \n";
}
else
...
my $mkvtreeProg = '/scratch/users/anniz44/bin/pro/vmatch-2.3.1-Linux_x86_64-64bit/mkvtree2';
if($mkvtreeProg){
...
my $vsubseqselectProg = '/scratch/users/anniz44/bin/pro/vmatch-2.3.1-Linux_x86_64-64bit/vsubseqselect2';
...
#--
my $fuzznuc = '/scratch/users/anniz44/bin/miniconda3/bin/fuzznuc';
if($fuzznuc){
...
my $needle = '/scratch/users/anniz44/bin/miniconda3/bin/needle';
if($needle){
...
while($seq = $seqIO->next_seq()){  # DC - replace 'next_seq' by 'next_seq()'
  ...
  if ($seqLength >= $seqMinSize){
	...
	#$seqID1 = $seqID1."_".$inputfileCount;
  	...
  	if($logOption){
  		print LOG "\n[$vmatch_hour:$vmatch_min:$vmatch_sec] /scratch/users/anniz44/bin/pro/vmatch-2.3.1-Linux_x86_64-64bit/vmatch2 @vmatchoptions\n"; # print in Logfile DC replaced vmatch by vmatch2
  	}
  	#Modification DC - 05/05/2017
  	#makesystemcall("./vmatch " . join(' ',@vmatchoptions)); #DC
  	makesystemcall("/scratch/users/anniz44/bin/pro/vmatch-2.3.1-Linux_x86_64-64bit/vmatch2 " . join(' ',@vmatchoptions)); #LK - DC replaced vmatch by vmatch2
...
sub casFinder
{
  ...
  eval{
    ...
    my $prokkaProg = '/scratch/users/anniz44/bin/miniconda3/envs/py37/bin/prokka';
    my $prodigalProg = '/scratch/users/anniz44/bin/miniconda3/envs/py37/bin/prodigal';
    ...
    my $macsyfinderProg = '/scratch/users/anniz44/bin/miniconda3/envs/py37/bin/macsyfinder';
    if($macsyfinderProg){
    ...
    my $prokka = "/scratch/users/anniz44/bin/miniconda3/envs/py37/bin/prokka $options";
    ...
    my $prodigal = "/scratch/users/anniz44/bin/miniconda3/envs/py37/bin/prodigal -i $inputfile -c -m -g $genCode -f gff -a $repProkka/$RefSeq.faa -o $repProkka/$RefSeq.gff ";
    if($quiet){ $prodigal .= "-q "; }
    ...
    if ( (-d $casfinder) and (-d $casdb) and (-d $profiles) ) {
	  #if($default){
	    $macsyfinder = "/scratch/users/anniz44/bin/miniconda3/envs/py37/bin/macsyfinder -w $cpuMacSyFinder -d $casdb -p $profiles --sequence-db $proteome all --out-dir $casDir ";
	    #$macsyfinder = "/scratch/users/anniz44/bin/miniconda3/envs/py37/bin/macsyfinder -w $cpuMacSyFinder -d $casdb -p $profiles --sequence-db $proteome all --out-dir $casDir ";
	    $macsyfinder .= $addToMacSy; # --multi-loci General-CAS
	  #}
	  #else{
	    #$macsyfinder = "/scratch/users/anniz44/bin/miniconda3/envs/py37/bin/macsyfinder -w $cpuMacSyFinder -d $casdb -p $profiles --sequence-db $proteome all --out-dir $casDir "; #--multi-loci 'CAS*' --min-genes-required 'CAS*' 1";
	  #}
    }
    #elsif($default){
        #$macsyfinder = "/scratch/users/anniz44/bin/miniconda3/envs/py37/bin/macsyfinder -w $cpuMacSyFinder --sequence-db $proteome all --out-dir $casDir"; #MacSyfinder without new definitions/profiles
    #}
    else{
    	$macsyfinder = "/scratch/users/anniz44/bin/miniconda3/envs/py37/bin/macsyfinder -w $cpuMacSyFinder --sequence-db $proteome all --out-dir $casDir"; #MacSyfinder without new definitions/profiles
    }
...
sub fastaAlignmentMuscle
{
  ...
  eval
  {
	my $prog = '/scratch/users/anniz44/bin/pro/muscle';

	my $muscle = "/scratch/users/anniz44/bin/pro/muscle -in $file -out $result "; #muscle command-line
	if ($quiet){
		$muscle .= " -quiet ";
	}
	#if ($fast){
...
sub extractsequence
{
  ...
  for($count = 0; $count < $#seqencesdefinition; $count+=2)
  {
	...
	# Modification DC - 05/05/2017
    	...
    	makesystemcall("/scratch/users/anniz44/bin/pro/vmatch-2.3.1-Linux_x86_64-64bit/vsubseqselect2 " . join(' ',@subselectoptions)); #LK - DC replaced vsubseqselect by vsubseqselect2
	my ($hour,$min,$sec) = Now();
	if($logOption){
   		print LOG "[$hour:$min:$sec] /scratch/users/anniz44/bin/pro/vmatch-2.3.1-Linux_x86_64-64bit/vsubseqselect2 @subselectoptions\n"; # DC replaced vsubseqselect by vsubseqselect2
	}
   }
  ...
  for($count=1; $count<= $#tabseq/2+1; $count++)
  {
	...
	#chdir(".."); #DC
   	...
   	while($c <= $#DR_cand_occ)
   	{
		...
		makesystemcall("/scratch/users/anniz44/bin/miniconda3/bin/fuzznuc " . join(' ',@fuzznucoptions)); #DC
		#my $testCrisprFile = "Test_".$crisprfile;
		#makesystemcall("cp $crisprfile $testCrisprFile ");
		my ($hour,$min,$sec) = Now();

		if($logOption){
			print LOG "[$hour:$min:$sec] /scratch/users/anniz44/bin/miniconda3/bin/fuzznuc " . join(' ',@fuzznucoptions)." \n";
		}
...
sub check_short_crispr
{
  ...
  $needleoptions = "/scratch/users/anniz44/bin/miniconda3/bin/needle -auto -asequence DR -bsequence $spacerfile -gapopen 10.0 -gapextend 0.5 -outfile alignDR_Spacer.needle"; # DC
...
sub callmkvtree
{
  ...
  {
    makesystemcall("pwd");
    makesystemcall("/scratch/users/anniz44/bin/pro/vmatch-2.3.1-Linux_x86_64-64bit/mkvtree2 -db $inputfile -dna -pl -lcp -suf -tis -ois -bwt -bck -sti1");
    my ($hour,$min,$sec) = Now();

    if($logOption){
    	print LOG "[$hour:$min:$sec] /scratch/users/anniz44/bin/pro/vmatch-2.3.1-Linux_x86_64-64bit/mkvtree2 -db $inputfile -dna -pl -lcp -suf -tis -ois -bwt -bck -sti1\n"; # DC replaced mkvtree by mkvtree2
    }
